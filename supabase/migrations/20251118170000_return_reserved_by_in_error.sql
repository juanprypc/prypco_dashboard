-- Return reserved_by information in reservation conflicts
-- This allows the frontend to know WHO owns the reservation
...
returns table(
  ...
  reserved_by_agent text  -- ADDED: Return who owns the reservation
) as $$
declare
...
begin
  ...
  if not found then
    return query select false, 'Unit not found'::text, p_unit_id, null::timestamptz, null::text;
    return;
  end if;

  -- Check if unit is released
  if v_released_status != 'Available' then
    return query select false, 'Unit not released'::text, p_unit_id, null::timestamptz, null::text;
    return;
  end if;

  -- Check if unit has stock
  if v_remaining_stock is null or v_remaining_stock <= 0 then
    return query select false, 'Unit sold out'::text, p_unit_id, null::timestamptz, null::text;
    return;
  end if;
  ...
  if v_current_reserved_by is not null
    ...
    -- CHANGED: Return who owns the reservation
    return query select false, 'Unit already reserved'::text, p_unit_id, v_current_expires_at, v_current_reserved_by;
    return;
  end if;
  ...
  if found then
    ...
    return query select false, 'Failed to acquire reservation lock'::text, p_unit_id, null::timestamptz, null::text;
  end if;
end;
$$ language plpgsql;

comment on function public.create_reservation(text, text, text, integer) is 
  'Atomically creates a reservation lock on a unit. Returns reserved_by_agent to help frontend distinguish between own reservation vs another agent.';
