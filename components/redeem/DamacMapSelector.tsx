...
// ↑ make desktop click detection tolerant on high‑DPI devices
const DRAG_SUPPRESS_THRESHOLD = 8;
...
export function DamacMapSelector({ catalogueId, selectedAllocationId, onSelectAllocation }: DamacMapSelectorProps) {
  ...
  const [containerSize, setContainerSize] = useState({ width: DEFAULT_BASE_WIDTH, height: DEFAULT_CONTAINER_HEIGHT });
  ...
  useEffect(() => { aspectRatioRef.current = imageAspectRatio; }, [imageAspectRatio]);
  ...
  // Natural ratio after image load
  ...
  useEffect(() => {
    ...
    fetch(`/api/damac/map${q}`)
      ...
      .then((data) => { setAllocations(data.allocations || []); setLoading(false); })
      .catch((err) => { console.error('Failed to fetch allocations:', err); setError('Failed to load units. Please try again.'); setLoading(false); });
  }, [catalogueId]);

  // If already cached by the browser, read natural size
  useEffect(() => {
    const img = imageRef.current;
    if (img && (img.complete || img.naturalWidth)) updateAspectRatioFromImage();
  }, []);
  ...
  useEffect(() => {
    const updateSize = () => {
      ...
      if (!el) return;
      setContainerSize({ width: el.clientWidth || DEFAULT_BASE_WIDTH, height: el.clientHeight || DEFAULT_CONTAINER_HEIGHT });
    };
    ...
    const g = globalThis as GlobalWithListeners;
    if (g.addEventListener && g.removeEventListener) {
      g.addEventListener('resize', updateSize);
      return () => g.removeEventListener?.('resize', updateSize);
    }
  }, []);

  // Filters / derived
  const brTypes = useMemo(() => {
    const types = new Set(allocations.map((a) => a.brType).filter((br): br is string => !!br && br.trim() !== ''));
  ...
  const filteredAllocations = useMemo(() => {
    ...
    return allocations.filter((a) => {
      const matchesSearch = !q || a.id.toLowerCase().includes(q) || a.damacIslandcode?.toLowerCase().includes(q) || a.brType?.toLowerCase().includes(q);
      const matchesType = filterType === 'all' || a.brType === filterType;
      return matchesSearch && matchesType;
    });
  }, [allocations, searchTerm, filterType]);

  const availableCount = useMemo(() => filteredAllocations.filter((a) => a.availability === 'available').length, [filteredAllocations]);
  const selectedAllocation = useMemo(() => allocations.find((a) => a.id === selectedAllocationId) || null, [allocations, selectedAllocationId]);
  ...
  const getScrollBounds = (targetZoom: number) => {
    const c = containerRef.current;
    if (!c) return { maxSL: 0, maxST: 0 };
    const bw = Math.max(containerSizeRef.current.width || DEFAULT_BASE_WIDTH, DEFAULT_BASE_WIDTH);
    const bh = bw * aspectRatioRef.current;
    const sw = bw * targetZoom;
    const sh = bh * targetZoom;
    return { maxSL: Math.max(0, sw - c.clientWidth), maxST: Math.max(0, sh - c.clientHeight) };
  };

  const setZoomAt = (oldZoom: number, newZoom: number, px: number, py: number) => {
    const c = containerRef.current;
    if (!c) return;
    const contentX = (c.scrollLeft + px) / oldZoom;
    const contentY = (c.scrollTop + py) / oldZoom;
    const { maxSL, maxST } = getScrollBounds(newZoom);
    c.scrollLeft = clamp(contentX * newZoom - px, 0, maxSL);
    c.scrollTop  = clamp(contentY * newZoom - py, 0, maxST);
  };

  // Discrete zoom (click / buttons / dblclick): double-rAF to avoid race with layout
  const applyZoomDiscrete = (getNext: (prev: number) => number, focusClientPoint?: Point) => {
    const c = containerRef.current;
    if (!c) return;
    const rect = c.getBoundingClientRect();
    const px = focusClientPoint ? focusClientPoint.x - rect.left : rect.width / 2;
    const py = focusClientPoint ? focusClientPoint.y - rect.top : rect.height / 2;

    const old = zoomRef.current;
    const next = clamp(getNext(old), MIN_ZOOM, MAX_ZOOM);
    if (Math.abs(next - old) < 1e-4) return;

    setZoom(next);
    // Ensure wrapper has resized before we adjust scroll
    requestAnimationFrame(() => {
      const c2 = containerRef.current; if (!c2) return;
      requestAnimationFrame(() => setZoomAt(old, next, px, py));
    });
  };

  // Continuous zoom (pinch / Ctrl+Wheel): single-rAF is fine
  const applyZoomContinuous = (targetZoom: number, focusClientPoint: Point) => {
    const c = containerRef.current; if (!c) return;
    const rect = c.getBoundingClientRect();
    const px = focusClientPoint.x - rect.left;
    const py = focusClientPoint.y - rect.top;

    const old = zoomRef.current;
    const next = clamp(targetZoom, MIN_ZOOM, MAX_ZOOM);
    if (Math.abs(next - old) < 8e-4) return;
  ...
  const handleResetZoom = () => {
    const c = containerRef.current;
    setZoom(1);
    if (c) c.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
  };

  // Desktop click / dblclick on the CONTAINER
  ...
  // Trackpad pinch (Ctrl+Wheel only)
  const handleWheel = (e: React.WheelEvent<HTMLDivElement>) => {
    if (!e.ctrlKey) return;      // never interfere with normal scroll
    e.preventDefault();
    const k = Math.exp(-e.deltaY * 0.0015);
    applyZoomContinuous(zoomRef.current * k, { x: e.clientX, y: e.clientY });
  };

  // Touch gestures (React for logic)
  const handleTouchStart = (e: React.TouchEvent<HTMLDivElement>) => {
    if (e.touches.length === 2) {
      ...
      if (now - lastTapRef.current < DOUBLE_TAP_DELAY) {
        e.preventDefault();
        const t = e.touches[0];
        applyZoomDiscrete((prev) => (prev >= MAX_ZOOM ? 1 : Math.min(MAX_ZOOM, prev + 1.5)), { x: t.clientX, y: t.clientY });
      }
  ...
  const handleTouchMove = (e: React.TouchEvent<HTMLDivElement>) => {
    if (e.touches.length !== 2 || !isPinchingRef.current || !initialPinchDistance.current) return;
    e.preventDefault();
    const [t0, t1] = [e.touches[0], e.touches[1]];
    const dist = Math.hypot(t1.clientX - t0.clientX, t1.clientY - t0.clientY);
    const scale = dist / initialPinchDistance.current;
    const target = clamp(initialZoom.current * scale, MIN_ZOOM, MAX_ZOOM);
    const cx = (t0.clientX + t1.clientX) / 2;
    const cy = (t0.clientY + t1.clientY) / 2;
    applyZoomContinuous(target, { x: cx, y: cy });
  };

  const handleTouchEnd = (e: React.TouchEvent<HTMLDivElement>) => {
    if (e.touches.length < 2) { isPinchingRef.current = false; initialPinchDistance.current = 0; }
  };

  // Desktop drag‑to‑pan
  const handlePointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    // Always clear suppression at the start of a sequence
    suppressClickRef.current = false;

    // Only start dragging if drag makes sense
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (!dragMovedRef.current && (Math.abs(dx) > DRAG_SUPPRESS_THRESHOLD || Math.abs(dy) > DRAG_SUPPRESS_THRESHOLD)) {
  ...
  const endPointerDrag = (pointerId: number) => {
    ...
    pointerDragRef.current = { active: false, pointerId: null, startX: 0, startY: 0, scrollLeft: 0, scrollTop: 0 };
    setIsDragging(false);
  };
  ...
  const handlePointerUp = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (dragMovedRef.current) suppressClickRef.current = true;   // swallow only the immediately-following click
    dragMovedRef.current = false;
  };
  ...
  // Native touch listeners to block iOS page zoom/double‑tap zoom, only on the map container
  useEffect(() => {
    const el = containerRef.current;
    if (!el) return;

    const onNativeTouchStart = (ev: TouchEvent) => {
      if (ev.touches.length > 1) ev.preventDefault(); // page pinch
      else {
        const now = Date.now();
        if (now - lastTapRef.current < DOUBLE_TAP_DELAY) ev.preventDefault(); // dbl‑tap zoom
      }
    };
          ...
          {!loading && !error && filteredAllocations.map((allocation) => {
            ...
            return (
              <button
                key={allocation.id}
                type="button"
                onClick={() => { if (!disabled) onSelectAllocation(selectedAllocationId === allocation.id ? null : allocation.id); }}
                disabled={disabled}
                className={
              ...
              >
                <div className="flex-1">
                  ...
                  <div className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-[var(--color-outer-space)]/60">
                    ...
                    {allocation.unitType && (<><span>•</span><span>{allocation.unitType}</span></>)}
                    {allocation.brType && (<><span>•</span><span>{allocation.brType}</span></>)}
                  </div>
                </div>
                <span className={'rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] ' + (allocation.availability === 'available' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700')}>
        ...
        <div className="rounded-[24px] border border-[#d1b7fb]/60 bg-white p-4">
          ...
          <div
            ...
            style={{
              ...
              touchAction: 'pan-x pan-y',  // keep one‑finger scroll; native listeners block only multi‑touch
          ...
          >
            <div
              ...
              style={{ width: baseWidth * zoom, height: baseWidth * imageAspectRatio * zoom }}
            >
              <img
                ref={imageRef}
                src={MAP_IMAGE}
            ...
            {selectedAllocation && (
              <div className="pointer-events-none absolute inset-0 hidden items-center justify-center p-8 lg:flex">
                <div className="pointer-events-auto max-w-md rounded-[20px] border border-[var(--color-outer-space)]/10 bg-white p-6 shadow-2xl">
                  ...
                  <button type="button" className="mt-5 w-full rounded-full bg-[var(--color-outer-space)] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#150f4c]">
                    Proceed to Payment
                  </button>
                </div>
              </div>
          ...
          <div className="fixed inset-x-0 bottom-0 z-50 lg:hidden">
            ...
            <div className="relative rounded-t-[28px] border-t border-[var(--color-outer-space)]/10 bg-white px-6 pb-8 pt-5 shadow-2xl">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-[var(--color-outer-space)]/50">Selected Unit</p>
                  <h3 className="mt-1 text-2xl font-bold text-[var(--color-outer-space)]">{selectedAllocation.damacIslandcode || selectedAllocation.id}</h3>
                </div>
                <button type="button" onClick={() => onSelectAllocation(null)} className="rounded-full p-2 text-[var(--color-outer-space)]/40 transition active:bg-[var(--color-panel)]" aria-label="Close">
              ...
              <div className="mt-5 grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <span className="mt-1.5 inline-block rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-700">Available</span>
                </div>
              </div>
